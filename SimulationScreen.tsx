...
// Main game view with dashboard (simplified vertical layout)
// ============================================
...
import { AssetControlPanel } from '../dashboard/AssetControlPanel';
import { PlaybackControls } from '../controls/PlaybackControls';
import { formatDuration } from '../../game/SimulationEngine';
import { LEGACY_STRATEGY_INFO as STRATEGY_INFO } from '../../game/DispatchStrategies';
...
export function SimulationScreen() {
  ...
  return (
    <div className={styles.container}>
      ...
      {/* Main dashboard - simplified vertical layout */}
      <main className={styles.dashboard}>
        {/* Top row: Gauge/Performance stacked on left, Charts on right */}
        <div className={styles.topRow}>
          ...
          <TimelineChart history={history} targetKw={targetKw}>
            {/* Playback controls or replay controls inside chart container */}
            {isReplayMode ? (
              <div className={styles.replayControls}>
                <div className={styles.replayInfo}>
                  ...
                  <span className={styles.replayStats}>
                    Final: {formatPower(latestResult?.achieved_kw ?? 0)} achieved
                    (Target: {formatPower(targetKw)})
                  </span>
                </div>
        ...
        {/* Asset Control Panel (Asset Status / Dispatch Intensity tabs) */}
        <AssetControlPanel
          assets={assets}
          dispatchIntensity={gameConfig.dispatchIntensity}
          enabledAssetTypes={gameConfig.asset_types}
          onIntensityChange={setDispatchIntensity}
        />
      </main>
    </div>
  );
